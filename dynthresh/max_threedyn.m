...
% max_threedyn.m
%
% script for maximizing the RR for a three trial dynamic threshold system.
%
...
[h1m,h2m,h3m,h13s,RRdyn,RRconst] = opti_threedyn(eps,D,TD);

figure(1), hold on, plot(eps,h13s,'k','linewidth',3);
...
ylabel('$\theta_{1:3}^{\rm max}$','fontsize',30,'interpreter','latex');
set(gca,'fontsize',30);

figure(2), hold on, plot(eps,RRconst,'k','linewidth',3);
hold on, plot(eps,RRdyn,'b','linewidth',8);
xlabel('$\epsilon$','fontsize',30,'interpreter','latex');
ylabel('${\rm RR}_{1:3}^{\rm max}(\epsilon)$','fontsize',30,'interpreter','latex');
set(gca,'fontsize',30);